...
public class luceneIndexador {
    ...
    public void beginIndex() throws IOException {
       ...
       /*for(tweetModel tweet:this.tweets){
            ...
            document.add(new StringField("id",tweet.getTweetID(), Field.Store.YES));
    ...
    public List<Document> getTweetsRelationated(String querystr) throws IOException, ParseException {
        ...
        if(querywords.length > 1){
            ...
            TopDocs docs = searcher.search(phraseQuery,30);
        ...
        //solo 1 palabra
            ...
            TopDocs docs = searcher.search(wildcardQuery,30);
            ScoreDoc[] hits = docs.scoreDocs;
            for(int i = 0; i < hits.length; i++){
                Document document = searcher.doc(hits[i].doc);
                tweets.add(document);
